...
impl ReaderIterator{
    ...
    pub(crate) fn new_with_upper_bound(file_name: String, upper_bound: i64, lower_bound: i64, index: u64, buffer: Buffer) -> ReaderIterator{
        let reader = Reader::new(file_name.as_str());
        ReaderIterator{
            reader,
            buffer,
    ...
    fn next(&mut self) -> Option<(i64, i64)>{
        ...
        if pair.0 < self.lower_bound{
            self.index = self.buffer.as_slice_pair().binary_search(&(self.lower_bound, 0)).unwrap_or_else(|x| x) as u64;
            return self.next();
        }
